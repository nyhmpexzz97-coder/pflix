...
export default class M3U8Downloader extends TypedEmitter {
    m3u8Url;
    output;
    segmentsDir;
    queue;
     ...
     */
    constructor(m3u8Url, output, options = {}) {
        super();
     ...
     */
    cancel() {
        if (["completed", "canceled", "error"].includes(this.status))
            return;
    ...
    async downloadSegment(tsUrl, index) {
        ...
        const segmentPath = path.resolve(this.segmentsDir, `segment${formattedIndex}.${this.options.suffix}`);
    ...
    async mergeTsSegments(total, deleteSource = true) {
        if (!this.isRunning())
            return;
        let mergedFilePath = path.resolve(this.segmentsDir, `output.${this.options.suffix}`);
        if (!this.options.convert2Mp4) {
        ...
        for (let index = 0; index < total; index++) {
            ...
            try {
                ...
                if (deleteSource)
                    await fs.unlink(segmentPath); // 删除临时 TS 片段文件
            }
            catch (error) {
    ...
    async cleanUpDownloadedFiles() {
        if (!this.options.clean)
            return;
        await Promise.all(this.downloadedFiles.map(async (file) => {
            try {
     ...
     */
    async convertToMp4(tsMediaPath) {
        if (!this.isRunning())
            return;
